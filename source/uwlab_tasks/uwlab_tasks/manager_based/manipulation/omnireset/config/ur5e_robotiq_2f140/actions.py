...
UR5E_ROBOTIQ_2F140_RELATIVE_OSC = RelCartesianOSCActionCfg(
    asset_name="robot",
...
UR5E_ROBOTIQ_2F140_RELATIVE_OSC_EVAL = RelCartesianOSCActionCfg(
    asset_name="robot",
    joint_names=["shoulder.*", "elbow.*", "wrist.*"],
    body_name="wrist_3_link",
    scale_xyz_axisangle=(0.1, 0.1, 0.1, 0.5, 0.5, 0.5),
...
UR5E_ROBOTIQ_2F140_RELATIVE_OSC_UNSCALED = RelCartesianOSCActionCfg(
    asset_name="robot",
    joint_names=["shoulder.*", "elbow.*", "wrist.*"],
    body_name="wrist_3_link",
...
class Ur5eRobotiq2f140RelativeOSCAction:
    ...
    arm = UR5E_ROBOTIQ_2F140_RELATIVE_OSC
    gripper = ROBOTIQ_GRIPPER_BINARY_ACTIONS


@configclass
class Ur5eRobotiq2f140RelativeOSCEvalAction:
    """Action config with high Kp gains (end-of-curriculum values) for eval / data-collection."""

    arm = UR5E_ROBOTIQ_2F140_RELATIVE_OSC_EVAL
    gripper = ROBOTIQ_GRIPPER_BINARY_ACTIONS


@configclass
class Ur5eRobotiq2f140SysidOSCAction:
